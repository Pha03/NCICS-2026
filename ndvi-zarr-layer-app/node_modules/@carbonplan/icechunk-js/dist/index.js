...
function deserializeMetadata(items, specVersion) {
  ...
  for (const item of items) {
    result[item.name] = specVersion === 1 /* V1_0 */ ? decode(item.value) : flexbuffers.toObject(
...
var ReadSession = class _ReadSession {
  ...
  constructor(storage, snapshot, specVersion) {
  ...
  static async open(storage, snapshotId, options) {
    ...
    return new _ReadSession(storage, snapshot, specVersion);
  }
  ...
  async loadManifest(manifestId, options) {
  ...
  async fetchChunkPayload(payload, options) {
    switch (payload.type) {
      case "inline":
        return payload.data;
      case "native": {
  ...
  async fetchChunkPayloadRange(payload, range, options) {
    let rangeStart;
    let rangeEnd;
...
function translateToHttpUrl(url) {
...
var Repository = class _Repository {
  ...
  async listBranches() {
    ...
    const branchFiles = /* @__PURE__ */ new Map();
    try {
  ...
  async checkoutBranch(name, options) {
    ...
    if (repoInfo) {
      ...
      return ReadSession.open(this.storage, snapshotId2, options);
    }
...
var IcechunkStore = class _IcechunkStore {
  session;
  transformRequest;
  constructor(session, transformRequest) {
  ...
  static async open(arg, options = {}) {
    if (arg instanceof ReadSession) {
      return new _IcechunkStore(arg, options.transformRequest);
    }
    ...
    return new _IcechunkStore(session, options.transformRequest);
  }
  /**
